...
def test_the_enabled_roster_is_exactly_what_has_been_probed(manifest):
    """Enabling a provider is a claim that a live call to it has succeeded.

    Cerebras and NVIDIA NIM both answer `GET /models` on this account and
    refuse every completion — a model list is what the platform serves, not
    what the key may call — so they stay off.
    """
    enabled = {n for n, p in manifest.providers.items() if p.enabled}
    assert enabled == {"groq", "gemini", "openrouter"}


def test_three_vendors_means_every_chain_has_somewhere_to_go(manifest):
    """Two vendors is the validated minimum; a third means a chain can still
    reach a vendor that has not already failed after the second one does."""
    for role in manifest.roles:
        vendors = {manifest.vendor_of(m) for m in manifest.chain(role)}
        assert len(vendors) >= 3, f"{role.value} reaches only {vendors}"
...
def test_groq_tpm_is_the_binding_constraint(manifest):
    ...
    # Both numbers come from the account's own x-ratelimit-* headers.
