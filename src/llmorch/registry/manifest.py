...
class ModelSpec(BaseModel):
    ...
    min_output_tokens: int = Field(default=0, ge=0)
    """Smallest max_tokens worth sending this model.

    Non-zero for models that spend hidden reasoning tokens out of the same
    budget as the visible answer: ask for 900 and the thinking eats all of it,
    and the reply comes back stopped at `length` with three lines in it. The
    tokens are only *reserved* at this size — commit reconciles to what was
    actually used — so a generous floor costs admission conservatism, not quota.
    """
...
def _validate(manifest: Manifest) -> None:
    ...
    # An output floor above the model's own cap is unsatisfiable.
    for m_ in manifest.models:
        if m_.min_output_tokens > m_.max_output:
            raise ManifestError(
                f"model {m_.id!r} sets min_output_tokens={m_.min_output_tokens} "
                f"above its max_output={m_.max_output}; no request could satisfy both"
            )
