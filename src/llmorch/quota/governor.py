...
class Governor:
    ...
    def sync_from_headers(self, model_id: str, snap: RateLimitSnapshot) -> None:
        ...
        for state, prov in self._states_for(model_id):
            ...
            if snap.remaining_requests is not None and rpd_spec and state.rpd:
                # Prefer the provider's own bucket size when it reports one: a
                # manifest limit can be stale, and for providers that publish
                # none it is only an estimate to begin with.
                bucket = snap.limit_requests or rpd_spec.value
                used = max(0, bucket - snap.remaining_requests)
    ...
    def restore_day_usage(
        ...
        *,
        ...
        """Seed today's counters from the persistent ledger.

        Quota is a property of the account, not of the process. A fresh run that
        started its counters at zero would believe it holds the whole daily
        allowance and walk straight into a wall that the previous run already
        built. Only the *daily* counters are restored: the per-minute windows
        are monotonic-clocked and cannot be carried across processes, and they
        drain in under a minute anyway.
        """
        provider = self.manifest.provider_of(model_id)
        scopes = {lim.scope for lim in provider.limits} or {LimitScope.MODEL}
        now_w = self.clock.now_utc()
        ...
        for scope in scopes:
            state = self._state(self._key(provider.name, model_id, scope), provider)
            if scope is LimitScope.ACCOUNT:
                requests, tokens = account_requests, account_tokens
            else:
                requests, tokens = model_requests, model_tokens
            if state.rpd is not None:
                state.rpd.set_count(now_w, requests)
            if state.tpd is not None:
                state.tpd.set_count(now_w, tokens)
