...
**One instruction becomes a runnable project, written by models from several
different vendors at once.**

You say *build a notes app with tags*. llmorch splits that into files, hands each
one to whichever model is best placed to write it, has a model from a *different*
vendor review what the first one wrote, checks that the finished files agree with
each other, then starts the thing and drives it over HTTP to find out whether it
actually runs. What lands on disk is a folder you can open and use, beside a
report saying which model wrote what and what the evidence for it is.

Splitting work across vendors is easy to say and awkward to do. This project is
mostly about the two things that make it awkward.

**The models cannot talk to each other.** Each one writes a single file, alone,
with no sight of the others and no memory between them. So before any of them
start, a planner writes an *interface contract* — the routes, the data shapes,
the page names, the runtime it all has to work under — and every model is handed
it verbatim. That contract is the only coordination there is, and it is the
reason a frontend written by one vendor works against a backend written by
another. Everything else in here exists to check that the contract was kept.

**Free tiers are rationed along several axes at once**, with different numbers
per provider, days that end at different midnights, and limits that are
sometimes shared across every model on an account. Get that arithmetic wrong and
you do not get a slightly worse result — you get a 429 in the middle of work you
have already partly paid for. So every request in the system, the planning
request included, goes through admission control before it is sent.

Nothing here needs an API key to try. `llmorch run "build a notes app"` runs the
whole pipeline against a mock provider, offline, and writes a folder at the end
of it.
...
The quota half is a library in its own right, and the part of this repository
with value outside it: `llmorch.quota` does the rationing arithmetic above and
has no idea the orchestrator exists.
